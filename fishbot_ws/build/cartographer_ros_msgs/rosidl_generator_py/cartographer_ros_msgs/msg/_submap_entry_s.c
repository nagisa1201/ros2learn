...
bool cartographer_ros_msgs__msg__submap_entry__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("cartographer_ros_msgs.msg._submap_entry.SubmapEntry", full_classname_dest, 51) == 0);
  }
  cartographer_ros_msgs__msg__SubmapEntry * ros_message = _ros_message;
  {  // trajectory_id
  ...
  {  // is_frozen
    PyObject * field = PyObject_GetAttrString(_pymsg, "is_frozen");
    if (!field) {
      return false;
    }
    assert(PyBool_Check(field));
    ros_message->is_frozen = (Py_True == field);
    Py_DECREF(field);
  }
...
PyObject * cartographer_ros_msgs__msg__submap_entry__convert_to_py(void * raw_ros_message)
{
  ...
  {  // is_frozen
    PyObject * field = NULL;
    field = PyBool_FromLong(ros_message->is_frozen ? 1 : 0);
    {
      int rc = PyObject_SetAttrString(_pymessage, "is_frozen", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
